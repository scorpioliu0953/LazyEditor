...
struct TimelineView: View {
            ...
            GeometryReader { outerGeo in
                HStack(spacing: 0) {
                    ...
                    ScrollView(.horizontal, showsIndicators: true) {
                        timelineContent
                            .frame(width: max(
                                outerGeo.size.width - Constants.trackLabelWidth,
                                vm.timelineVM.totalContentWidth(segments: vm.timeline.segments) + 60
                            ))
                    }
                }
            }
    ...
    private var playheadOverlay: some View {
        let x = playheadXPosition
        let totalH = totalTimelineHeight

        return PlayheadView(height: totalH)
            .offset(x: x - 5)
